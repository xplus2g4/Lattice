...
// Everything under _authed needs a session; /landing, /login, /invite and /auth stay public.
// A signed-out visitor at `/` goes to the landing page; anywhere else, to sign-in.
export const Route = createFileRoute('/_authed')({
  beforeLoad: async ({ location }) => {
    const user = await getSessionUser()
    if (!user) {
      if (location.pathname === '/') throw redirect({ to: '/landing' })
      throw redirect({
        to: '/login',
