...
Open `http://localhost:3000/landing` after `npm run dev`. This is a native
TanStack route for iterating on the existing marketing page; `/` remains the
course picker. It needs no API. Edit `src/components/landing/landing-page.tsx`
for copy and React interactions, and `src/components/landing/landing.css` for
styles scoped to this page. The concept map and proposed plans retain the
product-vision labels and informational interactions from `marketing/`.
...
it defaults to `http://localhost:3000`. Canonical, sharing, and social image URLs
use that origin. Public social crawlers need a deployed, reachable URL.

Brand and social images live in `public/landing-assets/`. The social images are
generated by `marketing/scripts/build.mjs`: run `npm ci` and `npm run build` in
`marketing/`, then copy `dist/assets/social-preview.png` and
`dist/assets/instagram-story.png` into `app/public/landing-assets/` when updating
them. The standalone GitHub Pages site in `marketing/` is still built separately;
changes to React copy/styles are not automatically mirrored there.
