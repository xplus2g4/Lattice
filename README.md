...
- The frontend uses the RPC API by default. Set `VITE_USE_MOCK_BACKEND=true` in `app/.env` only for an offline demo with prewritten answers and temporary data. Restart the app after changing this setting. With the API, application records survive restarts in Postgres; Cognee knowledge lives in its separate embedded store.
