...
- The similarity floor is a magic number: `MIN_SIMILARITY = 0.75` cosine similarity in `server/lattice/db/repo/course_summaries.py`, a guess made on 2026-09-24 for mean-pooled bge-small-en-v1.5 vectors without any measurement, there so that with few courses the three nearest are not unrelated ones. Measure it against a real course set before trusting it; [backlog.md](../wiki/backlog.md) carries the open question.
- bge-small embeds at most 512 tokens, hence one profile per Material pooled into a mean rather than one long summary. The `vector(384)` DDL pins the width; a wider model needs a migration.
