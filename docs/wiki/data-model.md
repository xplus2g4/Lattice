...
Every chunk Cognee stores for the global tier must carry `material_id`, `week`, `lecture_no`, and `page`/`slide` so a citation resolves to something a student can open. Private-tier chunks carry `note_id`. The mechanism (node sets, tags, or metadata on `add`) is verified against the pinned version. If the pinned version cannot attach metadata, the Worker pre-converts to markdown with per-page headers so the page number survives inside the text.
