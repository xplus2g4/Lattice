...
Course knowledge store, Stage 1 (Cognee-backed). This page describes the target Stage 1 system; `server/` is catching up to it. The [small ontology fixture](../research/2026-09-20-cognee-ontology-findings.md) exercises storage and traversal mechanisms, not production related-concept quality; real-course extraction, ranking and citation resolution remain unproven.
...
6. **Related courses by Course summary.** Every course with a ready Material has one pgvector row in the app Postgres, the mean of its Materials' embeddings, refreshed on a timer without an LLM. `/ask` also searches the global tier of the three nearest courses, as the instructor principal and under its own isolation map, and returns their answers as a `related` tier. [ADR 0008](../adr/0008-related-courses-from-summary-neighbours.md)

## What this architecture deliberately does not do

- No cross-course Sessions; a Session is always scoped to one course. Retrieval may reach the global tier of up to three Related courses ([ADR 0008](../adr/0008-related-courses-from-summary-neighbours.md)); nothing else crosses a course boundary.
