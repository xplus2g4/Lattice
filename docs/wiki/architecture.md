...
flowchart LR
  ...
  K --> G[("Kuzu<br/>embedded graph file")]
  K & A --> L["LLM / embeddings APIs"]
  A & W --> S[("GCS<br/>raw uploads · backups")]
```

A three-container monolith (web / api / worker) plus Postgres and Caddy on one GCP VM under docker compose. The API is the only component that knows about users; Cognee never sees a browser request or a token. The Worker is the API image with a different entrypoint, consuming a Postgres-table job queue.
...
1. **Two knowledge tiers, one query.** Each course has a global dataset (all enrolled readers) and per-user private datasets (owner only). A single `/ask` spans both. Isolation is enforced twice: Cognee dataset permissions and an API-level citation check, both exercised by a CI canary test. [ADR 0002](../adr/0002-two-tier-datasets-double-isolation.md)
