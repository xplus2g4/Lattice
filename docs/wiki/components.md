...
| **API** (FastAPI, in [`server/`](../../server/)) | `api` container | Auth verification, RBAC, course/enrolment/material/session records, the `/ask` pipeline with its Related-course lane, answer validation, rate limits, response cache, the Course-summary refresh timer (until a Worker exists) | Postgres, Cognee (in-process), embedding API (for Course summaries), LLM API, GCS, job queue | Run cognify inline (always via Worker) |
