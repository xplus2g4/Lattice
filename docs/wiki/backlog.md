...
Why this is still open: the [memory-engines evaluation](../research/memory-engines-evaluation.md) (Sep 2026) notes Kuzu was archived after acquisition upstream and has been deprecated in Graphiti. A pinned embedded file DB keeps working regardless, and it keeps ops to a single VM with file-level backups. But if the ecosystem keeps moving away from Kuzu, switching the profile default to Neo4j before launch may be cheaper than after. Decide before launch, with the canary tests as the gate either way.
...
These gate launch. Each is tracked in GitHub:
