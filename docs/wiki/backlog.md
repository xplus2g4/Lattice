...
5. **The prompt-injection canary: live validation pending.** [security.md](./security.md) describes the implemented context wrapping, deterministic prompt checks and real API canary against a poisoned Material and Note. The live test still needs a budgeted run with configured credentials; #14 also blocks real execution in CI. Prompt wrapping is not a guarantee against arbitrary injection or extraction-time knowledge poisoning.
